...
while True:
	...
	# loop over the facial & mask embeddings
	for (encoding,pred,box) in zip(encodings,preds,locs):
		...
		if True in matches:
			...
			if currentname != name:
				...
				#requests = send_message(name)
				#print ('Status Code: '+format(requests.status_code)) #200 status code means email sent successfully
